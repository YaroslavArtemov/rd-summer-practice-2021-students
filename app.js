        ...
        game.GameView = (function () {
            ...
            function getImages() {
                ...
                return {
                    ...
                    imgCoin: $('#imgCoin').get(0),
                    imgPolice: $('#imgPolice').get(0),
                    imgPoliceSelf: $('#imgPoliceSelf').get(0),
                    imgThief: $('#imgThief').get(0),
                    imgThiefSelf: $('#imgThiefSelf').get(0),
                    imgSwitch: $('#imgSwitch').get(0)
                };
            }
            function setMapCanvasSizing($canvas, width, height) {
                 ...
                 */
                return $('#game_area').css("width", width + "px")
            ...
            function getCanvasBuffer(width, height, map, cellSize) {
                var canvas = setMapCanvasSizing($('#game_area'), width, height).get(0);
            ...
            GameView.prototype.drawObject = function (ctx, objType, x, y, cellSize) {
                ...
                if (img) {
                    $('#game_area').get(0).getContext("2d").drawImage(img, cellSize * x + 2, cellSize * y + 2, cellSize - 4, cellSize - 4);
                }
            };
            ...
            GameView.prototype.updateMap = function (map) {
                ...
                if (!this.game.mapBuffer) {
                    ...
                    setMapCanvasSizing($('#game_area'), width, height);
                    this.game.mapBuffer = getCanvasBuffer(width, height, map, this.game.mapCellSize);
                }
                var ctx = $('#game_area').get(0).getContext("2d");
            ...
            GameView.prototype.setGameCaption = function (name, status) {
                ...
                this.game.$gameCaption
                    .empty()
                    .append($(app.utils.t(
                        //менять эту строку под нашу вёрстку, добавить статус игры и исправить ошибки в gameCaption
                        //"<div class='game-caption-name'>{name} <span class='game-caption-status game-caption-status-{status}'>{statusName}</span></div>",
                        "<div id = 'gameCaptionName'>{name}</div> <div id = 'gmaeCaptionStatus-{status}'>{statusName}</div>",
            ...
            GameView.prototype.getPlayer = function (player) {
                ...
                return $(app.utils.t(
                    ...
                    "</div>", 
                    */
                    "<div id = 'team1Users{playerId}' class='game-player game-player-status-{status}'>>" +
                    "<span class = 'playerName'>{name}</span>" +
                    "<span calss = 'playerLives'>{lives}</span>" +
                    "<span class = 'playerCoins'>{coins}</span>" +
                    "<span class = 'playerDeths'>{deaths}</span>" +
                    "</div>",
                    {
                        playerId: player.id,
                        status: status,
            ...
            GameView.prototype.setTeamCaption = function (team, $team) {
                ...
                $team.$caption
                    ...
                    .append(app.utils.t(
                        ...
                        "</div>", 
                        */
                        "<div class = 'team{role}Caption'>" +
                            "<span class = 'teamName'>{name}</span>" +
                            "<span class = 'teamRole'>{roleName}</span>" +
                        "</div>",
                        {
                            role: role,
            ...
            GameView.prototype.setButtons = function (status) {
                ...
                */
                ...
                if (this.state.status === GameApi.GameStatus.open || this.state.status === GameApi.GameStatus.ready) {
                    ...
                    else { //btns disable to connected user
                ...
                //While game
                if (this.state.status === GameApi.GameStatus.starting || this.state.status === GameApi.GameStatus.inProcess) {
                    btns.$btnStart.addClass("hidden");
                    btns.$btnLeave.addClass("hidden");
                    btns.$btnConnect.addClass("hidden");
                    btns.$btnConnectThief.addClass("hidden");
                    ...
                    if (isOwner) { //btns disable to Owner
                        ...
                        btns.$btnPause.addClass("hidden");
                        if (isAdmin) {//btns disable to Admin
                            btns.$btnCancel.removeClass("hidden");
                        } else {
                            btns.$btnCancel.addClass("hidden");
                        }
                    ...
                    if (isOwner) { //btns disable to Owner
                        ...
                        btns.$btnStart.addClass("hidden");
                        if (isAdmin) { //btns disable toAdmin
                            btns.$btnCancel.removeClass("hidden");
                        } else {
                            btns.$btnCancel.addClass("hidden");
                        }
                    }//btns disable to ordinary user
                    btns.$btnPause.addClass("hidden");
                    btns.$btnLeave.addClass("hidden");
                    btns.$btnConnect.addClass("hidden");
                    btns.$btnConnectThief.addClass("hidden");
